...
const useResizeEffect = (
  ...
  const updateSize = (deltaX: number, deltaY: number) => {
    ...
    if (status === ResizeHandleType.Resize) {
      designer.screen.setSize(startWidth + deltaX, startHeight + deltaY)
      container.current.scrollBy(
        containerRect.width + deltaX,
        containerRect.height + deltaY
      )
    } else if (status === ResizeHandleType.ResizeHeight) {
      designer.screen.setSize(startWidth, startHeight + deltaY)
      container.current.scrollBy(
        container.current.scrollLeft,
        containerRect.height + deltaY
      )
    } else if (status === ResizeHandleType.ResizeWidth) {
      designer.screen.setSize(startWidth + deltaX, startHeight)
      container.current.scrollBy(
        containerRect.width + deltaX,
  ...
  designer.subscribeTo(DragStartEvent, (e) => {
    if (!designer.workbench.currentWorkspace?.viewport) return
    ...
    if (target?.closest('*[data-designer-resize-handle]')) {
      ...
      designer.cursor.setStyle(getStyle(status))
  ...
  designer.subscribeTo(DragMoveEvent, (e) => {
    if (!designer.workbench.currentWorkspace?.viewport) return
    if (!status) return
...
export const ResponsiveSimulator: React.FC<IResponsiveSimulatorProps> =
  observer((props) => {
    ...
    useDesigner((designer) => {
      useResizeEffect(container, content, designer)
    })
    return (
      <div
        {...props}
